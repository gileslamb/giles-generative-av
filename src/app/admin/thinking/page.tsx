...
export default function AdminThinkingListPage() {
  ...
  const handleDelete = async (e: React.MouseEvent, id: string) => {
    ...
    setDeletingId(id)
    try {
      const res = await fetch(`/api/admin/thinking/${id}`, { method: 'DELETE' })
  ...
  return (
    <div className="p-8">
      <div className="flex items-center justify-between mb-8">
        <h1 className="text-xl font-mono tracking-wider text-white/70">Thinking</h1>
        <button onClick={() => router.push('/admin/thinking/new')} className="px-4 py-2 text-sm font-mono bg-white/10 border border-white/10 rounded hover:bg-white/15 transition-colors">New Entry</button>
      </div>
      ...
      <div className="border border-white/10 rounded-lg overflow-hidden">
        {loading ? (
          ...
          <table className="w-full">
            ...
            <tbody>
              {entries.map((entry) => (
                <tr key={entry.id} onClick={() => router.push(`/admin/thinking/${entry.id}`)} className="border-b border-white/10 hover:bg-white/5 cursor-pointer transition-colors">
                  <td className="px-4 py-3 text-sm text-white/90">{entry.title}</td>
                  <td className="px-4 py-3 text-sm text-white/40 font-mono">{entry.slug}</td>
